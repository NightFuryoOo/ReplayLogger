...
namespace ReplayLogger
{
    public partial class ReplayLogger : Mod, ICustomMenuMod, IGlobalSettings<ReplayLoggerSettings>
    {
        ...
        private const int BlockMaxAgeMs = 5000;
        ...
        private static int DeterminePantheonNumber(BossSequence sequence)
        {
            ...
            try
            {
                ...
                if (bossScenesField?.GetValue(sequence) is not BossScene[] bossScenes || bossScenes.Length == 0)
                {
                    return 0;
                }
        ...
        private void TrackHeroBoxActive(IEnumerable<HealthManager> bossList)
        {
        ...
        private void WriteHeroBoxActiveWarning(int currentState, IEnumerable<HealthManager> bossList, int? previousState)
        {
            long unixTime = DateTimeOffset.Now.ToUnixTimeMilliseconds();
            ...
            string warning = $"|{lastScene}|+{unixTime - lastUnixTime}{hpInfo}|HeroBoxActive: {message}";
            InvWarn?.Add(warning);
        }

        private void AppendHeroBoxOffDurationWarning()
        {
            ...
            string warning = $"|{lastScene}|+{unixTime - lastUnixTime}{hpInfo}|HeroBoxActive Off Duration: {duration.ToString("F3", CultureInfo.InvariantCulture)}s";
            InvWarn?.Add(warning);
        }
        ...
        private string BuildHeroBoxHpInfo(IEnumerable<HealthManager> bossList)
        {
            ...
            foreach (var boss in bossList)
            {
                ...
                hpInfo += $"|{entry.lastHP}/{entry.maxHP}";
            }
        ...
        private void FlushBufferedSectionsForTransition()
        {
            ...
            if (DamageAnfInv != null && DamageAnfInv.HasContent)
            {
        ...
        private void Close(bool manualClose)
        {
            ...
            try
            {
                ...
                if (File.Exists(currentNameLog))
                {
                    ...
                    string toastText = $"{namePrefix}: {Path.GetFileName(newPath)}";
                    SavedLogToast.Record(toastText);
                    SavedLogToast.Show(ReplayLogger.GetHudToastSeconds());
                    toastShown = true;
                }
            }
            catch (Exception ex)
            ...
            finally
            {
                ...
                {
                    customCanvas?.DestroyCanvasDelayed(ReplayLogger.GetHudToastSeconds());
                }
                else
                {
                    customCanvas?.DestroyCanvas();
